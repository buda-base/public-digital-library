...
export function EtextSearchBox(props) {
  ...
  const startSearch = useCallback(async (useQuery) => {
    ...
    res = res.map((r,i) => r.startPageCstart != null ? r : { ...r, startPageCstart: r.chunkCstart ?? r.highlightStart - 2500, startPnum: "--", endPnum: "--" })
    let last
  ...
  return <div ref={ref} class={"ETSBresult "+on} 
    ...
    >
      <div class="ETSBheader">
        <span>{I18n.t("punc.nth",{num: n+1})}{res.pname != undefined ? " ["+res.pname+"]" : ""}</span><span>{I18n.t("resource.pageRvol",{p:res.startPnum, v:res.volumeNumber, e:res.etextNumber})}</span>
      </div>
      <div>
        {label}
      </div>
