...
export function getPropLabel(that, i, withSpan = true, withLang = false, useI18n = "", useI18n_count = 1) {
...
export function top_right_menu(that,etextTitle,backUrl,etextres)
{
   ...
   if(etextTitle)
      ...
      <div class={"nav"+(onZhMirror?" zhMirror":"")+ (that.state.collapse.navMenu?" on":"")+(onKhmerServer||onKhmerUrl?" khmerServer":"")
         ...
         { !onZhMirror  && userGuideLink("guide") }
         ...
         { !onKhmerServer && <>
            ...
            <Popover 
               id="more-pages-menu"
               open={that.state.collapse.more} 
               transformOrigin={{ vertical: 'top', horizontal: 'left'}} 
               anchorOrigin={{vertical: 'top', horizontal: 'left'}} 
               anchorEl={that.state.anchor?.more} 
               onClose={e => { that.setState({...that.state,collapse: {...that.state.collapse, "more":false }})}}
               onClick={e => { that.setState({...that.state,collapse: {...that.state.collapse, "more":false }})}}
               style={{zIndex:10000000}}
            >
               <MenuItem>{aboutLink()}</MenuItem>
               <MenuItem>{newSearchLink()}</MenuItem>
               <MenuItem>{userGuideLink()}</MenuItem>
            </Popover>
         </>}
