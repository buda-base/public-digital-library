   ...
   renderData = (kZprop, iiifpres, title, otherLabels, div = "", hash = "") => {
      ...
      let data = kZprop.map((k) => {
            ...
            //if(!k.match(new RegExp("Revision|Entry|prefLabel|"+rdf+"|toberemoved"))) {
               ...
               (!k.match(new RegExp(adm+"|adm:|isRoot$|SourcePath|"+rdf+"|toberemoved|entityScore|associatedCentury|lastSync|dateCreated|qualityGrade|digitalLendingPossible|inRootInstance|workPagination|partIndex|partTreeIndex|legacyOutlineNodeRID|sameAs|thumbnailIIIFSe|instanceOf|instanceReproductionOf|instanceHasReproduction|seeOther|(Has|ction)Member$|serialHasInstance|withSameAs|hasNonVolumeParts|hasPartB|addIALink|first(Text|Vol)N?"+(this._dontMatchProp?"|"+this._dontMatchProp:"")))
      ...
      if(root && root.length) {
         inTitle  = [ 
            <h3 class="inT">
               ...
               <Link onClick={() => this.setState({opartinview:""})} to={"/show/"+shortUri(root[0].value)+"?part="+this.props.IRI} class="outL"><img src="/scripts/mirador/images/collecR.svg"/>{I18n.t("result.openO")}</Link>
            </h3>,
            //<br/>,
